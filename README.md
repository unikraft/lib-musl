...
The default configuration doesn't include complex number support, i.e. the `LIBMUSL_COMPLEX` option is disabled. 
...
in case of requiring complex number support, [`compiler-rt`](https://github.com/unikraft/lib-compiler-rt/) is to be included to the build. 
If strict compliance with the complex arithmetic requirements of ISO C (specifically Annex G) and IEEE 754 is not required,
passing the compiler flag [`-fcx-limited-range`](https://gcc.gnu.org/onlinedocs/gcc/Optimize-Options.html) removes the requirement for `lib-compiler-rt`.
